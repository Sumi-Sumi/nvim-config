...
local lazy_path = settings.lazy_path
...
function Lazy:load_plugins()
    ...
    local get_plugins_list = function()
        local list = {}
        local tmp = vim.split(fn.globpath(modules_dir, utils.joinpath("*", "plugins.lua")), "\n")
        local subtmp = vim.split(fn.globpath(modules_dir, utils.joinpath("*", "user", "plugins.lua")), "\n")
...
function Lazy:load_lazy()
    ...
    local lazy_settings = {
        root = utils.joinpath(data_dir, "lazy"),
        defaults = {
            lazy = true,
        },
        git = {
        ...
        performance = {
            cache = {
                enabled = true,
                path = utils.joinpath(cache_dir, "lazy", "cache"),
